...
The project outputs lightweight summary files and a self-contained HTML viewer for spatial review of cores, patches, and nuclei.
...
- spatially ordered patch grids
- clickable high-resolution patch popups
- boundary show/hide and opacity controls inside the patch popup

![Core detail view](docs/figures/viewer_core_detail_core7.png)

![Patch grid view](docs/figures/viewer_patch_grid_core1.png)

Selected patch example used for visual review: **Core 1, row 1, col 3**, containing **472 detected nuclei**.
...
5. Tile each core using overlapping patches.
6. Downsample 40x patches to approximately 20x-equivalent scale for StarDist.
...
9. Deduplicate detections across overlapping patch boundaries.
...
The selected review patch below is **Core 1, row 1, col 3**. The two distributions after it summarize nuclear mask size and patch-level nuclear load.

![Selected patch for histogram context](docs/figures/viewer_selected_patch_core1_row1_col3.png)

![Nuclear area histogram](docs/figures/nuclear_area_histogram.png)

![Patch load histogram](docs/figures/patch_load_histogram.png)
...
These limitations are why the HTML viewer is part of the deliverable: it makes core-level and patch-level review transparent rather than hiding the model's residual uncertainty.
...
- spatial patch grids that preserve tissue location
